...
// neutralizar los bloques grises
#pragma omp barrier
#pragma omp single
                {
                    std::size_t i = 0; std::size_t j = P - 1;

                    while (i < j) {
                        // ve si no hay bloques grises
                    ...
                    // encuentra pos del pivote
                    less_than = 0;
                    for (std::size_t k = 0; k < P; k++)
                        less_than += threads_left[k] - threads_start[k];

                    //pone el bloque gris que queda si existe en su pos final
